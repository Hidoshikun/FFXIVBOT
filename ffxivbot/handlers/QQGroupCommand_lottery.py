            ...
            if command == "create":
                ...
                try:
                    ...
                    if sec_command == "info":
                        ...
                        try:
                            assert lott.host_user == str(user_id) or user_info["role"] == "admin" or user_info[
                                "role"] == "owner", "您不是抽奖组织者或管理员，无权更改抽奖信息"
                            ...
                            msg = "抽奖\"#{}: {}\"已于{}强行终止".format(lott.id, lott.name, time.strftime(TIMEFORMAT,
                                                                                                   time.localtime(
                                                                                                       lott.end_time)))
                        ...
                        try:
                            ...
                            if len(mem_list) == 0:
                                ...
                                msg = "{} 已成功报名抽奖\"#{}: {}\"".format(
                                    " ".join(["[CQ:at,qq={}]".format(qq) for qq in mem_list]), lott.id, lott.name)
                        ...
                        try:
                            ...
                            msg = "抽奖\"#{}: {}\"已变更为可公开注册模式，请使用\"/lottery #{} register\"命令注册".format(lott.id, lott.name,
                                                                                                     lott.id)
                        ...
                        try:
                            ...
                            lott.public = False
                            lott.save(update_fields=["public"])
                            msg = "抽奖\"#{}: {}\"已变更为私密模式，请使用\"/lottery #{} user add/del @$user\"命令添加/删除指定用户".format(
                                lott.id, lott.name, lott.id)
