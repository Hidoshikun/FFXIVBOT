                    ...
                    if not qq.isdecimal():
                        ...
                        if len(msg_list) == 1:
                            if len(mems) == 0:
                                ...
                                if (len(vtlist["voted_by"]) >= group.ban_cnt):
                                    ...
                                    if (target_user_info["role"] == "owner"):
                                        msg = "[CQ:at,qq=%s] 虽然你是狗群主%s无法禁言，但是也被群友投票禁言，请闭嘴%s分钟[CQ:face,id=14]" % (
                                        qq, bot.name, mem.ban_time)
                                    if (target_user_info["role"] == "admin"):
                                        msg = "[CQ:at,qq=%s] 虽然你是狗管理%s无法禁言，但是也被群友投票禁言，请闭嘴%s分钟[CQ:face,id=14]" % (
                                        qq, bot.name, mem.ban_time)
                                    if (str(mem.user_id) == str(receive["self_id"])):
                                        msg = "%s竟然禁言了可爱的%s[CQ:face,id=111][CQ:face,id=111]好吧我闭嘴%s分钟[CQ:face,id=14]" % (
                                        voted_msg, bot.name, mem.ban_time)
                                    ...
                                    msg = "[CQ:at,qq=%s] 时长为%s分钟的禁言投票，目前进度：%s/%s" % (
                                    mem.user_id, mem.ban_time, len(vtlist["voted_by"]), group.ban_cnt)
                            ...
                            if not msg_list[1].isdecimal():
                                ...
                                if (len(mems) == 0):
                                    max_ban_time = 10
                                    ban_time = min(int(msg_list[1]), max_ban_time) if int(qq) != int(
                                        receive["self_id"]) else int(msg_list[1])
